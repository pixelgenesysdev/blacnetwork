...
class YITH_PayPal_Gateway extends WC_Payment_Gateway_CC {
	...
	public function get_login_url( $environment = '' ) {
		...
		$additional_arguments = apply_filters(
			'yith_paypal_payments_login_url_attributes',
			array(
				'channelId'          => 'partner',
				//'partnerId'          => $this->get_partner_id(),
				'product'            => YITH_PayPal::get_instance()->get_product_type(),
				'integrationType'    => 'FO',
		...
		$locale = YITH_PayPal::get_instance()->get_onboarding_language();
		if ( $locale ) {
			$additional_arguments['country.x']   = $locale;
			$additional_arguments['countryCode'] = $locale;
		}
	...
	public function get_method_title() {
		 ...
		 */
		$icons_to_show = apply_filters( 'yith_ppwc_method_title_icons', array( 'paypal', 'venmo', 'giropay', 'ideal' ) );
